...
- ADC FUERZA: ADS1219 (24-bit) leído por I2C desde el Analog Discovery   <-- reemplaza al NI 9205
    Cadena: celda -> INA-4LC (G=601) -> ADS1219 AIN0/AIN1 -> AD (maestro I2C)
- Acelerómetro: PCB 352C33 (100 mV/g) -> NI cDAQ-9174 con NI 9234 (IEPE)

TASAS:
- Fuerza:       500 Hz en UI/guardado (ADS1219 configurado internamente a 1000 SPS)
- Aceleración:  500 Hz configurada directamente en NI 9234
...
# Cargar el SDK dwf (WaveForms) para hablar I2C con el ADS1219 desde el Analog Discovery
try:
    from ctypes import cdll, c_int, c_double, c_uint, c_ubyte, byref
    _dwf = cdll.dwf if sys.platform.startswith("win") else cdll.LoadLibrary("libdwf.so")
    HAS_DWF = True
except Exception as _e:
    HAS_DWF = False
    _dwf = None
    print(f"⚠️ WaveForms SDK (dwf) no disponible - Fuerza en simulación: {_e}")

# ============================================
# CONFIGURACIÓN DEL HARDWARE
# ============================================

# ── FUERZA: celda -> INA-4LC (G=601) -> ADS1219 (24-bit) -> Analog Discovery (I2C) ──
FORCE_I2C_SCL  = 0           # DIO0 = SCL
FORCE_I2C_SDA  = 1           # DIO1 = SDA
ADS1219_ADDR8  = 0x40 << 1   # 7-bit 0x40 en formato 8-bit del SDK (confirma jumpers SparkFun)
ADS1219_VREF   = 2.048       # referencia interna (V)
ADS1219_GAIN   = 4           # x1 si te acercas a la capacidad de la celda; x4 para fuerzas pequeñas
ADS1219_SPS    = 1000        # 20/90/330/1000 — 1000 es el máximo del ADS1219
# Config reg0: MUX=AIN0-AIN1(000) | GAIN | DR=1000(11) | CM=continuo(1) | VREF=interno(0)
...
ACCEL_SAMPLE_RATE = FORCE_SAMPLE_RATE
...
class ForceAcquisitionThread(threading.Thread):
    """FUERZA: celda DYMH-105 -> INA-4LC (G=601) -> ADS1219 (24-bit) -> Analog Discovery (I2C).

    El AD es maestro I2C; el ADS1219 hace la conversión a 24 bits. Empuja el voltaje
    medido (salida del INA) a force_queue en modo crudo, igual que la versión NI 9205,
    así el resto de la GUI sólo necesita conocer FORCE_SAMPLE_RATE.
    """
    def __init__(self):
        super().__init__(daemon=True)
        self.running = True
        self.hdwf = None
        self.samples_per_read = 25   # muestras a acumular antes de encolar

    def _i2c_write(self, *data):
        tx = (c_ubyte * len(data))(*data); nak = c_int()
        ok = _dwf.FDwfDigitalI2cWrite(self.hdwf, c_int(ADS1219_ADDR8), tx, c_int(len(data)), byref(nak))
        if not ok or nak.value != 0:
            raise RuntimeError(f"ADS1219 no responde por I2C en 0x{ADS1219_ADDR8 >> 1:02X} (NAK={nak.value})")
        return True

    def _i2c_write_read(self, cmd, n):
        tx = (c_ubyte * 1)(cmd); rx = (c_ubyte * n)(); nak = c_int()
        ok = _dwf.FDwfDigitalI2cWriteRead(self.hdwf, c_int(ADS1219_ADDR8), tx, c_int(1), rx, c_int(n), byref(nak))
        if not ok or nak.value != 0:
            raise RuntimeError(f"ADS1219 no responde por I2C en 0x{ADS1219_ADDR8 >> 1:02X} al leer 0x{cmd:02X} (NAK={nak.value})")
        return rx

    def _data_ready(self):
        return (self._i2c_write_read(0x24, 1)[0] & 0x80) != 0   # status: bit 7 = DRDY

    def _read_volts(self):
        rx = self._i2c_write_read(0x10, 3)                      # RDATA -> 3 bytes
        raw = (rx[0] << 16) | (rx[1] << 8) | rx[2]
        if raw & 0x800000:
            raw -= 1 << 24
        return raw / (1 << 23) * (ADS1219_VREF / ADS1219_GAIN)  # voltios en la entrada del ADC

    def run(self):
        if not HAS_DWF:
            print("Error fuerza (ADS1219/AD): WaveForms SDK no disponible; no se inicia simulación automática")
            return
        try:
            self.hdwf = c_int()
            _dwf.FDwfDeviceOpen(c_int(-1), byref(self.hdwf))
            if self.hdwf.value == 0:
                print("No se pudo abrir el Analog Discovery (¿WaveForms abierto en otro lado?)")
                self.hdwf = None
                return

            # Motor I2C por hardware del AD
            _dwf.FDwfDigitalI2cReset(self.hdwf)
            _dwf.FDwfDigitalI2cRateSet(self.hdwf, c_double(400e3))
            _dwf.FDwfDigitalI2cSclSet(self.hdwf, c_int(FORCE_I2C_SCL))
            _dwf.FDwfDigitalI2cSdaSet(self.hdwf, c_int(FORCE_I2C_SDA))
            free = c_int()
            ok_clear = _dwf.FDwfDigitalI2cClear(self.hdwf, byref(free))
            if not ok_clear:
                raise RuntimeError("No se pudo inicializar/limpiar el bus I2C del Analog Discovery")
            if not free.value:
                raise RuntimeError("Bus I2C ocupado: revisa alimentación 3.3V, GND común y pull-ups SDA/SCL")

            # Inicializar ADS1219: RESET, WREG config, START continuo
            self._i2c_write(0x06); time.sleep(0.001)   # RESET
            self._i2c_write(0x40, ADS1219_CONFIG)      # WREG config
            cfg = self._i2c_write_read(0x20, 1)[0]
            if cfg != ADS1219_CONFIG:
                raise RuntimeError(f"ADS1219 respondió, pero config leída 0x{cfg:02X} != esperada 0x{ADS1219_CONFIG:02X}")
            self._i2c_write(0x08)                      # START/SYNC (continuo)

            t0 = time.time()
            while self.running and not self._data_ready():
                if time.time() - t0 > 2.0:
                    raise RuntimeError("ADS1219 configurado, pero no entrega DRDY después de 2 s")
                time.sleep(0.0002)
            first_sample = self._read_volts()
            print(f"✅ ADS1219 vía AD-I2C: INA G=601, ADC G={ADS1219_GAIN}, ADC {ADS1219_SPS} SPS, UI objetivo {FORCE_SAMPLE_RATE} Hz")

            # Pacing a FORCE_SAMPLE_RATE: deadline absoluto + busy-wait final.
            # time.sleep en Windows redondea hacia arriba en sub-ms, así que se
            # duerme el grueso y se afina el último tramo con espera activa. El
            # ADS1219 por I2C tiene techo ~515 Hz, esto lo acota a ~500 Hz; la
            # tasa REAL lograda se mide en la GUI (timestamp por batch) y se guarda.
            target_interval = 1.0 / FORCE_SAMPLE_RATE
            ...
            while self.running:
                ...
                batch.append(self._read_volts())
                if len(batch) >= self.samples_per_read:
                    t_last = time.perf_counter()
                    arr = np.asarray(batch, dtype=float); batch = []
                    if not force_queue.full():
                        force_queue.put((arr, t_last))
        except Exception as e:
            print(f"Error fuerza (ADS1219/AD): {e}")
        finally:
            if self.hdwf is not None:
                try:
                    self._i2c_write(0x02)              # POWERDOWN
                    _dwf.FDwfDeviceClose(self.hdwf)
                except Exception:
                    pass

    def _run_simulation(self):
        """Modo simulación sin Analog Discovery / ADS1219"""
        t = 0
        while self.running:
            dt = self.samples_per_read / FORCE_SAMPLE_RATE
            t_arr = np.linspace(t, t + dt, self.samples_per_read)
            data = 0.5 * np.sin(2 * np.pi * 40 * t_arr) + 0.02 * np.random.randn(self.samples_per_read)
            if not force_queue.full():
                force_queue.put((data, time.perf_counter()))
            t += dt
            time.sleep(dt * 0.9)
...
class CaracterizacionFuerzaGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("🔬 Caracterización Sensor de Fuerza DYMH-105 (ADS1219 + AD)")
    ...
    def update_plots(self):
        ...
        if self.experiment_mode != "ACCEL_ONLY":
            while not force_queue.empty():
                try:
                    arr, t_batch = force_queue.get_nowait()
    ...
    def _save_raw_recording(self):
        ...
        if nf_raw != na_raw:
            print(
                f"ADVERTENCIA: fuerza/aceleración con distinta longitud "
                f"({nf_raw}/{na_raw}); se exportan {n_common} muestras comunes."
            )

        force_arr = force_arr[:n_common]
        accel_arr_0 = accel_arr_0[:n_common]

        fs_force_eff = self._force_fs_eff()   # tasa REAL medida de fuerza (no se asume 500)
        ...
        # Fuerza @ tasa REAL medida (no se asume 500 Hz → eje de tiempo correcto)
        nf = len(force_arr)
        tf = np.arange(nf) / fs_force_eff
        df_f = pd.DataFrame({'tiempo_s': tf, 'fuerza_V': force_arr})
        force_file = base + "_fuerza.csv"
        force_path = os.path.join(DATOS_DIR, force_file)
        df_f.to_csv(force_path, index=False, sep='\t')

        # Aceleración @ tasa configurada
        na = len(accel_arr_0)
        ta = np.arange(na) / ACCEL_SAMPLE_RATE
        df_a = pd.DataFrame({
            'tiempo_s': ta,
            'aceleracion_sensor_g': accel_arr_0[:na],
        })
        accel_file = base + "_accel.csv"
        accel_path = os.path.join(DATOS_DIR, accel_file)
        df_a.to_csv(accel_path, index=False, sep='\t')

        doc_file = "datos_doc_" + base + "_tiempo_fuerza_aceleracion.txt"
        doc_path = os.path.join(DATOS_DIR, doc_file)
        df_doc = pd.DataFrame({
            'tiempo_s': tf,
            'fuerza_V': force_arr,
            'aceleracion_g': accel_arr_0[:nf],
        })
        df_doc.to_csv(doc_path, index=False, sep='\t')

        # Sidecar enlazando ambos archivos
        sidecar = {
            'file_force': force_file,
            'file_accel': accel_file,
            'file_doc_txt': doc_file,
            'timestamp': timestamp,
            'fs_hz_force': round(float(fs_force_eff), 2),        # tasa REAL medida de fuerza
            'fs_hz_force_nominal': FORCE_SAMPLE_RATE,            # objetivo de pacing
            'fs_hz_force_eff': round(float(fs_force_eff), 2),    # = fs_hz_force (explícito)
            'fs_hz_accel': ACCEL_SAMPLE_RATE,
            'sync_policy': 'force_paced_to_nominal_real_fs_measured; buffers_trimmed_to_common_length',
            'n_samples_force_raw': int(nf_raw),
            'n_samples_accel_raw': int(na_raw),
            'n_samples_force': int(nf),
            'n_samples_accel': int(na),
            'duration_s_force': round(float(nf / fs_force_eff), 3),
            'duration_s_accel': round(float(na / ACCEL_SAMPLE_RATE), 3),
            'accel_sensitivity_mV_g': ACEL_SENSIBILIDAD_MV_G,
            ...
            'notas': self.notas_edit.text(),
            'canal_fuerza': 'ADS1219 24-bit via Analog Discovery I2C (INA-4LC G=601, ADC G=%d)' % ADS1219_GAIN,
            'canal_aceleracion': 'ai0 bancada (NI 9234)',
        }
        json_path = os.path.join(DATOS_DIR, base + "_meta.json")
        with open(json_path, 'w', encoding='utf-8') as jf:
            _json.dump(sidecar, jf, indent=2, ensure_ascii=False)

        QMessageBox.information(self, "Guardado",
            f"Datos guardados:\n{force_path}\n{accel_path}\n{doc_path}\n{json_path}\n\n"
            f"Fuerza: {nf} muestras @ {fs_force_eff:.1f} Hz REAL (obj {FORCE_SAMPLE_RATE}) ({nf/fs_force_eff:.2f}s)\n"
            f"Acel:   {na} muestras @ {ACCEL_SAMPLE_RATE} Hz ({na/ACCEL_SAMPLE_RATE:.2f}s)\n"
            f"Raw antes de sincronizar: fuerza {nf_raw}, acel {na_raw}")

        self.status_label.setText(f"💾 Guardado: {force_file} + {accel_file} + {doc_file}")
...
if __name__ == "__main__":
    ...
    print("="*60)
    print(f"   Fuerza: ADS1219 24-bit vía Analog Discovery I2C (INA G=601, ADC G={ADS1219_GAIN})")
    print(f"           DIO{FORCE_I2C_SCL}=SCL, DIO{FORCE_I2C_SDA}=SDA, addr 0x{ADS1219_ADDR8>>1:02X}, {FORCE_SAMPLE_RATE} SPS")
    ...
    print(f"   ADS1219 SCL/SDA -> DIO{FORCE_I2C_SCL}/DIO{FORCE_I2C_SDA} del AD (3.3V, pull-ups 4.7k)")
    print("   Requiere WaveForms instalado (dwf.dll en PATH) y el AD libre.")
    print("")
