...
int main(int argc, char **argv)
{
    ...
    const char *port_name = (argc >= 2 && argv[1] && argv[1][0]) ? argv[1] : "/dev/ttyS0";
    fd = open(port_name, O_RDWR | O_NOCTTY | O_SYNC);
    if (fd < 0) {
        printf("Error %d al abrir %s: %s\n", errno, port_name, strerror(errno));
        return 1;
    }
    ...
    tty.c_cc[VMIN]  = 1;                           // read 1 char at a time
    ...
    while(1) 
	{
                ...
                while(1) 
		{
            ...
            if(n <= 0)
            	continue; // Si no hay datos o hay error, reintentar
            else
			{
				if(flagcom!=0)
        			flagcom++;
				...
				if(flagcom==5)
   				{
    				...
    				// NOTA: Para cambiar a modo SENSORLESS, modificar el PID arriba
    				// para usar y_obs en lugar de y en el cálculo de ef
